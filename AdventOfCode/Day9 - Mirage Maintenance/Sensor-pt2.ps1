...
for ($lineIdx = 0; $lineIdx -lt $file.Length; $lineIdx++)
{
    ...
    foreach ($sequence in $sequences)
    {
        ...
        $sequence = [array]::Reverse($sequence);
    }

    Write-Verbose "Reversed Sequence Input #$lineIdx :"
    foreach ($sequence in $sequences)
    {
        Write-Verbose "$sequence";
    }
    ...
    for ($sequenceIdx = $sequences.Length - 2; $sequenceIdx -ge 0; $sequenceIdx--)
    {
        ...
        $a = $sequences[$sequenceIdx + 1][-1];
        $b = $sequences[$sequenceIdx][-1] - $a;

        $sequences[$sequenceIdx] += $b;
    }
