...
for ([int]$row = 0; $row -lt $file.Count; $row++) 
{
    ...
    {
        if (-not [System.Char]::IsLetter($file[$row][$col]) -and
            -not [System.Char]::IsDigit($file[$row][$col]) -and
            $file[$row][$col] -ne '.')
        {
            Write-Host "Found '$($file[$row][$col])'! There should be a part number nearby!"
            ...
            for ([int]$tempRow = $row - 1; $tempRow -le ($row + 1); $tempRow++) 
            {
                ...
                for ([int]$tempCol = $col - 1; $tempCol -le ($col + 1); $tempCol++)
                {
                    ...
                    if ([System.Char]::IsDigit($file[$tempRow][$tempCol]))
                    {
                        ...
                        if ($end -le $start) {
                            Write-Host "Error at $tempRow : $start >= $end"
                        }
